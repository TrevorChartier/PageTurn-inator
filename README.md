...
Features smooth, interpolated servo movements (__move_smooth) to ensure the page turns gracefully without tearing the sheet music.

* **Hardware/Software Integration**: Runs on Raspberry Pi 5, interfaces with a Raspberry Pi Camera (via Picamera2) requesting a steady 50 FPS, and drives a physical servo motor using the PCA9685 controller.
